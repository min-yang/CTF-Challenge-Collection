...
- [shell](shell)：栈溢出利用，覆盖filename字符串指针，然后输入对应文件中存在的用户名密码通过验证拿到shell
- [nobug](nobug)：格式化字符串漏洞，但是涉及逆向能力，存在格式化字符串漏洞的函数的跳转入口反编译工具识别不出来，需要自己定位
- [magic](magic)：数组负数下标越界利用，需要_IO_FILE结构体达到任意读和任意写的效果，从而拿到shell，相关知识需要进一步学习掌握
- [play](play)：该挑战主要考察逆向能力，只要分析清楚程序的逻辑，找到办法打赢怪兽，即可直接利用栈溢出漏洞拿到shell
